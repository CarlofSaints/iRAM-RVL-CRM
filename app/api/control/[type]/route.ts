...
const VALID_TYPES: ControlType[] = ['clients', 'stores', 'products', 'reps', 'warehouses', 'channels', 'sites'];
...
function permForType(type: ControlType): string {
  // Channels and the site-control master are part of the stores feature — gate
  // under manage_stores so we don't need a new permission key (and a re-seed).
  if (type === 'channels' || type === 'sites') return 'manage_stores';
  return `manage_${type}`;
}
